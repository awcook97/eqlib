...
/*0x08*/
	...
	ItemIndex& operator=(const ItemIndex& other)
	{
		memcpy(m_slots, &other.m_slots, sizeof(m_slots));
		return *this;
	}

	short GetTopSlot() const { return m_slots[0]; }

	void SetSlot(int depth, short value)
	{
		if (depth < MAX_INVENTORY_DEPTH)
			m_slots[depth] = value;
	}

	short GetSlot(int depth) const
	{
		if (depth < MAX_INVENTORY_DEPTH)
	...
	short& operator[](int slot) { return m_slots[slot]; }

	bool IsValid() const { return m_slots[0] != -1; }
	bool IsBase() const { return m_slots[0] != -1 && m_slots[1] == -1; }
	bool IsInContainer() const { return m_slots[0] != -1 && m_slots[1] != -1; }

	// Remove one depth to get index representing whatever the parent is.
	ItemIndex GetParentIndex() const
	{
		ItemIndex copy = *this;
...
public:
	...
	ItemGlobalIndex CreateItemGlobalIndex(ItemContainerInstance location, int slot0, int slot1 = -1, int slot2 = -1) const
	{
		return ItemGlobalIndex(location, CreateItemIndex(slot0, slot1, slot2));
	}

	ItemContainerInstance GetContainerType() const { return m_type; }
	int GetAtDepth() const { return m_atDepth; } // depth of this container within a parent container
...
public:
	...
	int GetSize(int depth = 0) const { return m_size; }
	EQLIB_OBJECT int GetRecursiveSize(int depth) const;

	// Retrieves the number of items in the container.
	uint32_t GetCount(int beginSlot, int endSlot, int depth) const
	{
	...
	uint32_t GetCount(int depth = 0) const
	{
...
public:
	...
	/*0x64*/
		...
		int get_TimerID() const { return RecastTime; }
...
/*0x30c*/
	...
	int GetSpellId(ItemSpellTypes type) const { return type < ItemSpellType_Max ? Spells[type].SpellID : 0; }
	uint8_t GetSpelllRequiredLevel(ItemSpellTypes type) const { return type < ItemSpellType_Max ? Spells[type].RequiredLevel : 0; }
	eItemEffectType GetSpellEffectType(ItemSpellTypes type) const { return type < ItemSpellType_Max ? Spells[type].EffectType : ItemEffectProc; }
	int GetSpellEffectiveCasterLevel(ItemSpellTypes type) const { return type < ItemSpellType_Max ? Spells[type].EffectiveCasterLevel : 0; }
	int GetSpellMaxCharges(ItemSpellTypes type) const { return type < ItemSpellType_Max ? Spells[type].MaxCharges : 0; }
	int GetSpellCastTime(ItemSpellTypes type) const { return type < ItemSpellType_Max ? Spells[type].CastTime : 0; }
	int GetSpellRecastTime(ItemSpellTypes type) const { return type < ItemSpellType_Max ? Spells[type].RecastTime : 0; }
	int GetSpellRecastType(ItemSpellTypes type) const { return type < ItemSpellType_Max ? Spells[type].RecastType : 0; }
	int GetSpellChanceProc(ItemSpellTypes type) const { return type < ItemSpellType_Max ? Spells[type].ProcRate : 0; }
	const char* GetOverrideName(ItemSpellTypes type) const { return type < ItemSpellType_Max ? Spells[type].OverrideName : ""; }
	int GetOverrideDesc(ItemSpellTypes type) const { return type < ItemSpellType_Max ? Spells[type].OverrideDesc : 0; }
	SpellData* GetSpellData(ItemSpellTypes type) { return type < ItemSpellType_Max ? &Spells[type] : nullptr; }
...
public:
	...
	bool IsSocketValid(int socket) const { return Sockets[socket].Type != 0; }
	int GetSocketType(int socket) const { return Sockets[socket].Type; }
	int IsSocketVisible(int socket) const { return Sockets[socket].bVisible; }
	int IsSocketInfusable(int socket) const { return Sockets[socket].bInfusible; }
...
constexpr size_t ItemDefinition_size = 0x688; // @sizeof(ItemDefinition) :: 2026-04-15 (live) @ 0x1402239f4
...
/*0x640*/ uint8_t             Unknown0x640[0x48]; // Apr 2026: new 72-byte sub-object (sync handle)
/*0x688*/
	...
	// Type in ItemClient.
	uint8_t get_ItemType() { return ItemClass; }
	__declspec(property(get = get_ItemType)) uint8_t ItemType;

	int GetMinLuck() const { return MinLuck; }
	int GetMaxLuck() const { return MaxLuck; }
...
#undef ITEMSPELLS_ACCESSOR
	...
	ITEMSPELLS& get_Mount() { return *reinterpret_cast<ITEMSPELLS*>(SpellData.GetSpellData(ItemSpellType_Blessing)); }
		__declspec(property(get = get_Mount)) ITEMSPELLS Mount;
	ITEMSPELLS& get_Illusion() { return *reinterpret_cast<ITEMSPELLS*>(SpellData.GetSpellData(ItemSpellType_Blessing)); }
		__declspec(property(get = get_Illusion)) ITEMSPELLS Illusion;
	ITEMSPELLS& get_Familiar() { return *reinterpret_cast<ITEMSPELLS*>(SpellData.GetSpellData(ItemSpellType_Blessing)); }
		__declspec(property(get = get_Familiar)) ITEMSPELLS Familiar;
	ITEMSPELLS& get_Keyring() { return *reinterpret_cast<ITEMSPELLS*>(SpellData.GetSpellData(ItemSpellType_Blessing)); }
		__declspec(property(get = get_Familiar)) ITEMSPELLS Keyring;

	uint32_t get_SkillMask(int idx) { return SpellData.SkillMask[idx]; }
	...
	static bool get_Summoned() { return false; }
...
// @end: ItemBase Members
	...
	// Common accessors
	int GetID() const { return GetItemDefinition()->ItemNumber; }
	uint8_t GetType() const { return GetItemDefinition()->Type; }
	uint8_t GetItemClass() const { return GetItemDefinition()->ItemClass; }
	const char* GetName() const { return GetItemDefinition()->Name; }
	bool IsContainer() const { return GetType() == ITEMTYPE_PACK; }
	int GetIconID() const { return GetItemDefinition()->IconNumber; }

	// current and max item counts (number in the stack)
	int GetMaxItemCount() const { return GetItemDefinition()->StackSize; }
	int GetItemCount() const { return StackCount; }
	bool IsStackable() const { return GetItemDefinition()->StackSize > 1; }
	...
	ItemContainer& GetHeldItems() { return Contents; }
	const ItemContainer& GetHeldItems() const { return Contents; }
	const ItemGlobalIndex& GetItemLocation() const { return GlobalIndex; }
	bool IsEmpty() const { return Contents.IsEmpty(); }
	ItemClient* GetContent(int index) { return GetHeldItem(index).get(); }
	...
	int GetLoreGroup() const { return GetItemDefinition()->Lore; }

	int GetMoneyValue() const { return GetItemDefinition()->Cost; }
	int GetTributeValue() const { return GetItemDefinition()->Favor; }
	int GetGuildTributeValue() const { return GetItemDefinition()->GuildFavor; }
	int GetPointTheme() const { return GetItemDefinition()->LDTheme; }
	int GetPointCost() const { return GetItemDefinition()->LDCost; }
	int GetPointType() const { return GetItemDefinition()->LDType; }

	int GetDelay() const { return GetItemDefinition()->Delay; }
	int GetDamage() const { return GetItemDefinition()->Damage; }
	bool CanWear(int slot) const { return (GetItemDefinition()->EquipSlots & (1 << slot)) != 0; }
	bool IsWearable() const { return GetItemDefinition()->EquipSlots != 0; }
	...
	int GetDamageShieldMitigation() const { return 0; }
	int GetDamShield() const { return 0; }
	int GetDoTShielding() const { return 0; }
	int GetShielding() const { return 0; }
	int GetSpellShield() const { return 0; }
	...
	int GetHeroicSvDisease() const { return 0; }
	int GetHeroicSvPoison() const { return 0; }
	int GetHeroicSvCorruption() const { return 0; }
	...
	int GetConvertItemID() const { return 0; } // ConvertItemID removed in April 2026
	CXStr GetConvertItemName() const { return ConvertItemName; }

	// Luck Accessors
	int GetLuck() const { return Luck; }
	int GetMinLuck() const { return GetItemDefinition()->GetMinLuck(); }
	int GetMaxLuck() const { return GetItemDefinition()->GetMaxLuck(); }
	...
	int get_EvolvingGroupID() { return pEvolutionData ? pEvolutionData->GroupID : 0; }
	__declspec(property(get = get_EvolvingGroupID)) int GroupID;

	int get_EvolvingCurrentLevel() { return pEvolutionData ? pEvolutionData->EvolvingCurrentLevel : 0; }
	__declspec(property(get = get_EvolvingCurrentLevel)) int EvolvingCurrentLevel;

	double get_EvolvingExpPct() { return pEvolutionData ? pEvolutionData->EvolvingExpPct : 0.0; }
	__declspec(property(get = get_EvolvingExpPct)) double EvolvingExpPct;

	int get_EvolvingMaxLevel() { return pEvolutionData ? pEvolutionData->EvolvingMaxLevel : 0; }
	...
	DEPRECATE("Use GetItemLocation instead of GetGlobalIndex")
		const ItemGlobalIndex& GetGlobalIndex() const { return GlobalIndex; }

	DEPRECATE("Use GetItemDefinition() instead of accessing Item2 directly")
	ItemDefinition* get_Item2() { return GetItemDefinition(); }
	__declspec(property(get = get_Item2)) ItemDefinition* Item2;
};

constexpr size_t ItemClient_size = 0x120; // @sizeof(ItemClient) :: 2026-04-15 (live) @ 0x1401ebae9
...
inline void* adopt_existing_shared_ptr_control_block(const ItemClient& ptr)
{
	return (void*)(reinterpret_cast<const uint8_t*>(&ptr) - 0x10);
}
